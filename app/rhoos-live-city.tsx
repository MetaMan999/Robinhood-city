...
const SAVE_KEY = "rhoos-city-hooktech-v2";
const TAU = Math.PI * 2;
...
function initialEngine(): Engine {
  ...
  return {
    player: { x: 970, y: 944, angle: -Math.PI / 2 },
    simMinutes: 6 * 60 + 52,
    day: 1,
    speed: 1,
    paused: false,
...
export default function RhoosLiveCity() {
  ...
  const interact = useCallback(() => {
    ...
    if (
      ...
      engine.activeJob.working = true;
      addEvent(engine, `${activeJob.title} shift authenticated.`);
    } else {
  ...
  function acceptJob(job: Job) {
    if (engine.reputation < job.requiredReputation || engine.activeJob) return;
  ...
  function toggleAudio() {
    ...
    oscillator.frequency.value = audioOn ? 180 : 520;
      ...
      <section className="live-stage">
        ...
        <header className="live-topbar">
          ...
          <div className="top-actions">
            <button onClick={toggleAudio}>{audioOn ? "AUDIO ON" : "AUDIO OFF"}</button>
            <button
              onClick={() => {
        ...
        {introOpen && (
          <div className="live-intro">
            ...
            <div className="intro-protocol">
              ...
              <strong>RHOOS</strong>
              <strong>CITY</strong>
              <p>FIRST PERSON ECONOMIC PROTOCOL</p>
            </div>
            <div className="intro-brief">
              <span className="panel-label">DISTRICT ONE / 06:52</span>
              <h1>The city is no longer a map.<br />You live inside the machine.</h1>
              <p>
                Walk its streets. Meet its workers. Take a shift. Buy a business.
                Install HookTech modules that move value whenever the city acts.
              </p>
              ...
              <small>A LIVING PROTOTYPE / LOCAL PLAYER STATE / VERSION 0.2</small>
            </div>
          </div>
        )}
      </section>
